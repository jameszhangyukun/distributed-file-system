...
public class RequestWrapper {
    ...
    public void sendResponse(MessageLite response) {
        ...
        NettyPacket nettyResponse = NettyPacket.buildPacket(body,
                PacketType.getEnum(request.getPacketType()));

        List<NettyPacket> responses =
                nettyResponse.partitionChunk(request.isSupportChunked(), Constants.CHUNKED_SIZE);
